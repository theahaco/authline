...
contract `eurcv_auth` via Soroban RPC.
...
soroban contract bindings ts \
  ...
  --contract-id CB2DHZMQHQE3TGUMD6BRM7UCJZNIPKDRVEQOWBIRRS3G2FZOGDTRKSB3 \
  --output-dir ./path/to/eurcv_auth
```
...
```json
"dependencies": {
  "eurcv_auth": "./path/to/this/folder"
}
```
...
```json
"scripts": {
  "postinstall": "soroban contract bindings ts --rpc-url https://soroban-rpc.mainnet.stellar.gateway.fm --network-passphrase \"Public Global Stellar Network ; September 2015\" --id CB2DHZMQHQE3TGUMD6BRM7UCJZNIPKDRVEQOWBIRRS3G2FZOGDTRKSB3 --name eurcv_auth"
}
```
...
```js
import { Contract, networks } from "eurcv_auth"
